...
def find_hits(agent, targets, z_extra=0, extra_jump_time=0):
    ...
    while i < struct.num_slices:
        ...
        intercept_time = struct.slices[i].game_seconds + sec_extra
        time_remaining = intercept_time - agent.time
        if time_remaining > 0:
            ball_location = Vector3(struct.slices[i].physics.location.x, struct.slices[i].physics.location.y, struct.slices[i].physics.location.z + z_extra)
            ball_velocity = Vector3(struct.slices[i].physics.velocity).magnitude()

            if abs(ball_location.y) > 5250:
                        ...
                        if forward_flag:
                            if ball_location[2] > 325 and slope > 1 and cap(ball_location[2]-400, 100, 2000) * 0.1 < agent.me.boost:
                                if abs((car_to_ball / forward_time) - agent.me.velocity).magnitude() - 300 < 400 * forward_time:
                                    should_dodge_aerial = ball_location[2] < 400
                                    hits[pair].append(aerial_shot(ball_location, intercept_time, best_shot_vector, slope, dodge_aerial=should_dodge_aerial, z_extra=z_extra))
                                    
                            if (ball_location[2] <= 300 or (not in_field(ball_location, 200) and not in_field(agent.me.location, 100))) and slope > 0.0:
                                hits[pair].append(
                                    jump_shot(ball_location, intercept_time, best_shot_vector, slope, z_extra=z_extra, extra_jump_time=extra_jump_time))
                            
                        elif backward_flag and ball_location[2] <= 280 and slope > 0.25:
                            hits[pair].append(
                                jump_shot(ball_location, intercept_time, best_shot_vector, slope, -1, z_extra=z_extra, extra_jump_time=extra_jump_time))
        else:
            i += 1
    return hits
