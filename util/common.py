...
def get_angle(p1, p2):
    d = (p1 * safe_div(p1.magnitude()) - p2 * safe_div(p2.magnitude())).magnitude()
...
def defaultPD(agent, local_target, direction=1.0, up=Vector3(0, 0, 1), no_roll=False):
    ...
    up = agent.me.local(up)  # where "up" is in local coordinates
    target_angles = [
